...
class DOS():
    ...
    def get_tdos(self):
        if self.spin:
            ...
            self.tdos = self.tdos.reshape(-1,self.nedos, order='F')*np.array([1,-1])[:,None]
        if not self.spin:
            self.tods = np.genfromtxt(self.file, skip_header=6, max_rows=self.nedos, usecols=1).reshape(-1,self.nedos,order='F')
